...
void TMTarragonController::autoSaveAndCloseCurrentJob()
{
    ...
    if (m_jobDataLocked) {
        ...
        QString currentDrop = m_dropNumberDDbox ? m_dropNumberDDbox->currentText() : "";
        
        if (!currentJobNumber.isEmpty() && !currentYear.isEmpty() && !currentMonth.isEmpty() && !currentDrop.isEmpty()) {
            outputToTerminal(QString("Auto-saving current job %1 (%2-%3-D%4) before opening new job")
                           .arg(currentJobNumber, currentYear, currentMonth, currentDrop), Info);
