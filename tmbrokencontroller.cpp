...
public:
    FormattedSqlModel(QObject *parent, QSqlDatabase db, TMBrokenController *ctrl)
...
private:
    TMBrokenController *controller;
};

TMBrokenController::TMBrokenController(QObject *parent)
    : BaseTrackerController(parent)
    , m_dbManager(DatabaseManager::instance())
    , m_fileManager(nullptr)
    , m_tmBrokenDBManager(TMBrokenDBManager::instance())
...
{
    if (!m_tmBrokenDBManager->initializeDatabase()) {
        qWarning() << "[TMBrokenController] Failed to initialize database. Job persistence will be unavailable.";
    ...
    m_fileManager = new TMBrokenFileManager(settings, this);
...
TMBrokenController::~TMBrokenController()
{
    if (m_autoSaveTimer) {
        m_autoSaveTimer->stop();
    }
    Logger::instance().info("TMBrokenController destroyed");
}

void TMBrokenController::initializeUI(
...
{
    Logger::instance().info("Initializing TM BROKEN APPOINTMENTS UI elements");
    ...
    if (m_tracker) {
        ...
        connect(m_tracker, &QTableView::customContextMenuRequested, this,
                &TMBrokenController::showTableContextMenu);
    }
    ...
    Logger::instance().info("TM BROKEN APPOINTMENTS UI initialization complete");
}

void TMBrokenController::connectSignals()
{
    // Connect buttons with null pointer checks
    if (m_openBulkMailerBtn) {
        connect(m_openBulkMailerBtn, &QPushButton::clicked, this, &TMBrokenController::onOpenBulkMailerClicked);
    }
    if (m_runInitialBtn) {
        connect(m_runInitialBtn, &QPushButton::clicked, this, &TMBrokenController::onRunInitialClicked);
    }
    if (m_finalStepBtn) {
        connect(m_finalStepBtn, &QPushButton::clicked, this, &TMBrokenController::onFinalStepClicked);
    }

    // Connect toggle buttons with null pointer checks
    if (m_lockBtn) {
        connect(m_lockBtn, &QToolButton::clicked, this, &TMBrokenController::onLockButtonClicked);
    }
    if (m_editBtn) {
        connect(m_editBtn, &QToolButton::clicked, this, &TMBrokenController::onEditButtonClicked);
    }
    if (m_postageLockBtn) {
        connect(m_postageLockBtn, &QToolButton::clicked, this, &TMBrokenController::onPostageLockButtonClicked);
    }

    // Connect dropdowns with null pointer checks
    if (m_yearDDbox) {
        connect(m_yearDDbox, QOverload<const QString &>::of(&QComboBox::currentTextChanged),
                this, &TMBrokenController::onYearChanged);
    }
    if (m_monthDDbox) {
        connect(m_monthDDbox, QOverload<const QString &>::of(&QComboBox::currentTextChanged),
                this, &TMBrokenController::onMonthChanged);
    }
    ...
    if (m_postageBox) {
        ...
        connect(m_postageBox, &QLineEdit::editingFinished, this, &TMBrokenController::formatPostageInput);
    ...
    if (m_countBox) {
        connect(m_countBox, &QLineEdit::textChanged, this, &TMBrokenController::formatCountInput);
    ...
    if (m_scriptRunner) {
        connect(m_scriptRunner, &ScriptRunner::scriptOutput, this, &TMBrokenController::onScriptOutput);
        connect(m_scriptRunner, &ScriptRunner::scriptFinished, this, &TMBrokenController::onScriptFinished);
    }
    ...
    if (m_jobNumberBox) {
        connect(m_jobNumberBox, &QLineEdit::editingFinished, this, [this]() {
            ...
            if (newNum != m_cachedJobNumber) {
                saveJobState();
                TMBrokenDBManager::instance()->updateLogJobNumber(m_cachedJobNumber, newNum, year, month);
    ...
    Logger::instance().info("TM BROKEN APPOINTMENTS signal connections complete");
}

void TMBrokenController::setupInitialUIState()
{
    Logger::instance().info("Setting up initial TM BROKEN APPOINTMENTS UI state...");
    ...
    Logger::instance().info("Initial TM BROKEN APPOINTMENTS UI state setup complete");
}

void TMBrokenController::setupDropWindow()
{
    if (!m_dropWindow) {
        return;
    }
    ...
    QString targetDirectory = "C:/Goji/TRACHMAR/BROKEN APPOINTMENTS/INPUT ZIP";
    ...
    connect(m_dropWindow, &DropWindow::filesDropped,
            this, &TMBrokenController::onFilesDropped);
    connect(m_dropWindow, &DropWindow::fileDropError,
            this, &TMBrokenController::onFileDropError);
    ...
    Logger::instance().info("TM BROKEN APPOINTMENTS drop window setup complete");
}

void TMBrokenController::populateDropdowns()
{
    Logger::instance().info("Populating TM BROKEN APPOINTMENTS dropdowns...");
    ...
    Logger::instance().info("TM BROKEN APPOINTMENTS dropdown population complete");
}

void TMBrokenController::setupOptimizedTableLayout()
{
    if (!m_tracker) return;
...
void TMBrokenController::updateControlStates()
{
...
// Button handlers
void TMBrokenController::onOpenBulkMailerClicked()
{
    if (!m_jobDataLocked) {
...
void TMBrokenController::onRunInitialClicked()
{
    if (!m_jobDataLocked) {
...
void TMBrokenController::onEditButtonClicked()
{
    if (!m_jobDataLocked) {
...
void TMBrokenController::onPostageLockButtonClicked()
{
    if (!m_jobDataLocked) {
...
void TMBrokenController::onJobNumberChanged()
{
    updateControlStates();
}

void TMBrokenController::onPostageChanged()
{
...
void TMBrokenController::onCountChanged()
{
...
void TMBrokenController::onYearChanged(const QString& year)
{
    Q_UNUSED(year)
    updateControlStates();
}

void TMBrokenController::onMonthChanged(const QString& month)
{
    Q_UNUSED(month)
    updateControlStates();
}

void TMBrokenController::onAutoSaveTimer()
{
    if (m_jobDataLocked || m_postageDataLocked) {
        saveJobState();
...
void TMBrokenController::onFilesDropped(const QStringList& filePaths)
{
...
void TMBrokenController::onFileDropError(const QString& errorMessage)
{
    outputToTerminal(QString("File drop error: %1").arg(errorMessage), Warning);
}

// Script handlers
void TMBrokenController::onScriptOutput(const QString& output)
{
...
void TMBrokenController::setTextBrowser(QTextBrowser* textBrowser)
{
    m_textBrowser = textBrowser;
}

// BaseTrackerController implementation methods
QTableView* TMBrokenController::getTrackerWidget() const
{
    return m_tracker;
}

QSqlTableModel* TMBrokenController::getTrackerModel() const
{
    return m_trackerModel;
}

QStringList TMBrokenController::getTrackerHeaders() const
{
    return {"JOB", "DESCRIPTION", "POSTAGE", "COUNT", "AVG RATE", "CLASS", "SHAPE", "PERMIT"};
}

QList<int> TMBrokenController::getVisibleColumns() const
{
    return {1, 2, 3, 4, 5, 6, 7, 8};
}

QString TMBrokenController::formatCellData(int columnIndex, const QString& cellData) const
{
...
QString TMBrokenController::formatCellDataForCopy(int columnIndex, const QString& cellData) const
{
...
void TMBrokenController::saveCurrentJobData()
{
    // Implementation
}

void TMBrokenController::loadJobData()
{
    // Implementation
}

bool TMBrokenController::validateJobData() const
{
...
bool TMBrokenController::validatePostageData()
{
...
bool TMBrokenController::validateJobNumber(const QString& jobNumber) const
{
    return jobNumber.length() == 5 && jobNumber.toInt() > 0;
}

bool TMBrokenController::validateMonthSelection(const QString& month) const
{
...
QString TMBrokenController::convertMonthToAbbreviation(const QString& monthNumber) const
{
...
QString TMBrokenController::getJobDescription() const
{
    return "TM BROKEN APPOINTMENTS";
}

bool TMBrokenController::hasJobData() const
{
    return validateJobData();
}

void TMBrokenController::updateJobDataUI()
{
...
void TMBrokenController::updateLockStates()
{
    updateControlStates();
}

void TMBrokenController::lockInputs(bool locked)
{
    m_jobDataLocked = locked;
    updateControlStates();
}

void TMBrokenController::enableEditMode(bool enabled)
{
    if (enabled) {
        m_jobDataLocked = false;
        m_postageDataLocked = false;
...
void TMBrokenController::updateTrackerTable()
{
    if (m_trackerModel) {
        m_trackerModel->select();
    }
...
void TMBrokenController::loadHtmlFile(const QString& resourcePath)
{
    if (!m_textBrowser) return;
...
TMBrokenController::HtmlDisplayState TMBrokenController::determineHtmlState() const
{
    return m_jobDataLocked ? InstructionsState : DefaultState;
}

void TMBrokenController::formatPostageInput()
{
...
void TMBrokenController::formatCountInput(const QString& text)
{
...
// >>> BEGIN NAS PATCH
void TMBrokenController::parseScriptOutput(const QString& line)
{
...
// >>> BEGIN NAS PATCH
void TMBrokenController::showNASLinkDialog(const QString& nasPath)
{
    if (nasPath.isEmpty()) {
    ...
    TMBrokenNetworkDialog* dialog = new TMBrokenNetworkDialog(
...
bool TMBrokenController::createExcelAndCopy(const QStringList& headers, const QStringList& rowData)
{
    return BaseTrackerController::createExcelAndCopy(headers, rowData);
}

void TMBrokenController::showTableContextMenu(const QPoint& pos)
{
...
void TMBrokenController::onCopyRow()
{
    QString result = copyFormattedRow();
    outputToTerminal("Copy Row: " + result, result.contains("success") ? Success : Warning);
    Logger::instance().info("TM BROKEN APPOINTMENTS: Copy row action triggered");
}

void TMBrokenController::loadJobState() {
    if (!m_databaseAvailable) {
        qWarning() << "[TMBrokenController] Cannot load job state: database not available.";
        return;
    }
    if (!m_tmBrokenDBManager) return;
    QString year = m_yearDDbox ? m_yearDDbox->currentText() : "";
    QString month = m_monthDDbox ? m_monthDDbox->currentText() : "";
    if (year.isEmpty() || month.isEmpty()) return;
    QVariantMap jobData = m_tmBrokenDBManager->loadJobData(year, month);
    if (!jobData.isEmpty()) {
...
void TMBrokenController::addLogEntry()
{
    if (!m_tmBrokenDBManager) {
    ...
    // Default values for TM BROKEN APPOINTMENTS
    QString classAbbrev = "STD"; // Standard class
    QString permitShort = "1662"; // Correct permit for TM BROKEN APPOINTMENTS
    ...
    // Add to database using the TM BROKEN APPOINTMENTS database manager
    if (m_tmBrokenDBManager->addLogEntry(logEntry)) {
...
QString TMBrokenController::calculatePerPiece(const QString& postage, const QString& count) const
{
...
void TMBrokenController::saveJobState() {
    if (!m_tmBrokenDBManager) return;
    QString year = m_yearDDbox ? m_yearDDbox->currentText() : "";
    QString month = m_monthDDbox ? m_monthDDbox->currentText() : "";
    if (year.isEmpty() || month.isEmpty()) return;
...
void TMBrokenController::saveJobToDatabase()
{
    QString jobNumber = m_jobNumberBox ? m_jobNumberBox->text() : "";
    QString year = m_yearDDbox ? m_yearDDbox->currentText() : "";
    QString month = m_monthDDbox ? m_monthDDbox->currentText() : "";
    ...
    if (m_tmBrokenDBManager->saveJob(jobNumber, year, month)) {
...
bool TMBrokenController::loadJob(const QString& year, const QString& month) {
    if (!m_tmBrokenDBManager) return false;
    QVariantMap jobData = m_tmBrokenDBManager->loadJobData(year, month);
    if (!jobData.isEmpty()) {
...
void TMBrokenController::resetToDefaults()
{
...
void TMBrokenController::debugCheckTables()
{
    // Implementation
}

void TMBrokenController::autoSaveAndCloseCurrentJob()
{
    ...
    if (m_jobDataLocked) {
        ...
        if (!currentJobNumber.isEmpty() && !currentYear.isEmpty() && !currentMonth.isEmpty()) {
            ...
            if (m_fileManager) {
                if (m_fileManager->moveFilesToHomeDirectory(currentYear, currentMonth)) {
                    outputToTerminal("Files moved successfully to HOME directory", Success);
                } else {
