...
original_data = {}
original_path = r"C:\Users\JCox\Downloads\original.csv"
if os.path.exists(original_path):
    df_original = pd.read_csv(original_path)
    df_filtered = df_original[
        ~df_original.iloc[:,0].str.contains('TOTAL', case=False, na=False) &
        ~df_original.iloc[:,1].str.contains('COUNT', case=False, na=False) &
        df_original.iloc[:,0].str.startswith('RAC', na=False) &
        pd.to_numeric(df_original.iloc[:,1], errors='coerce').notna()
    ]
    
    for idx, row in df_filtered.iterrows():
        version = row.iloc[0]
        count = int(row.iloc[1].replace(',', ''))  # Handle comma-formatted numbers
        if version in original_data:
            original_data[version] += count
        else:
            original_data[version] = count
...
# Compare totals before writing CSV
if results:
    # Calculate current totals by group
    current_totals = {
        'CBC2': 0, 'CBC3': 0, 'EXC': 0, 
        'INACTIVE A-PO': 0, 'INACTIVE A-PU': 0,
        'NCWO 1-A': 0, 'NCWO 1-AP': 0, 
        'NCWO 2-A': 0, 'NCWO 2-AP': 0,
        'PREPIF': 0
    }
    ...
    for idx, row in final_df.iterrows():
        if pd.notna(row['Count']):
            value = row['Value']
            count = row['Count']
            if 'CBC2' in value: current_totals['CBC2'] += count
            elif 'CBC3' in value: current_totals['CBC3'] += count
            elif 'EXC' in value: current_totals['EXC'] += count
            elif 'A-PO' in value: current_totals['INACTIVE A-PO'] += count
            elif 'A-PU' in value: current_totals['INACTIVE A-PU'] += count
            elif '1-A' in value and not 'AP' in value: current_totals['NCWO 1-A'] += count
            elif '1-AP' in value: current_totals['NCWO 1-AP'] += count
            elif '2-A' in value and not 'AP' in value: current_totals['NCWO 2-A'] += count
            elif '2-AP' in value: current_totals['NCWO 2-AP'] += count
            elif 'PREPIF' in value: current_totals['PREPIF'] += count

    # Calculate original totals by group
    original_totals = {k: 0 for k in current_totals.keys()}
    for version, count in original_data.items():
        # CBC2 specific matching
        if 'RAC2404-DM07-CBC2' in version:
            original_totals['CBC2'] += count
        # CBC3 specific matching
        elif 'RAC2401-DM03' in version and 'CBC2' not in version:
            original_totals['CBC3'] += count
        # EXC specific matching
        elif 'RAC2406-DM03-RACXW' in version:
            original_totals['EXC'] += count
        # INACTIVE specific matching
        elif 'RAC2501-DM06' in version:
            if '-PO' in version:
                original_totals['INACTIVE A-PO'] += count
            elif '-PU' in version:
                original_totals['INACTIVE A-PU'] += count
        # NCWO specific matching
        elif 'RAC2501-DM05' in version:
            if 'NCWO1-A' in version and 'AP' not in version:
                original_totals['NCWO 1-A'] += count
            elif 'NCWO1-AP' in version:
                original_totals['NCWO 1-AP'] += count
            elif 'NCWO2-A' in version and 'AP' not in version:
                original_totals['NCWO 2-A'] += count
            elif 'NCWO2-AP' in version:
                original_totals['NCWO 2-AP'] += count
        # PREPIF specific matching
        elif 'RAC2404-DM06-PPIF' in version:
            original_totals['PREPIF'] += count

    # Display differences with whole numbers
    print("\nCOUNT DIFFERENCES FROM INPUT TO OUTPUT")
    for group in current_totals.keys():
        diff = int(current_totals[group] - original_totals[group])
        if diff > 0:
            print(f"{group}: {diff} [ERROR!]")
        else:
            print(f"{group}: {diff}")
