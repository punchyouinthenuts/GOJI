...
def process_zip_files(transaction):
    ...
    # Validate file extensions after extraction
    for root, _, files in os.walk(filebox_path):
        for file in files:
            _, ext = os.path.splitext(file)
            if ext and ext.lower() != '.txt':
                print(f"FILE {file} IS NOT A TXT FILE! SCRIPT WILL TERMINATE, PRESS ANY KEY TO CONTINUE...")
                input()
                sys.exit(1)
            elif not ext:
                ...
                transaction.log_operation('rename', old_path, new_path)

    # Move validated files to destinations
    for root, _, files in os.walk(filebox_path):
        for file in files:
...
def process_ncwo_input(transaction):
    ...
    for file in os.listdir(input_dir):
        if file.endswith(".txt"):
            ...
            transaction.log_operation('modify', file_path)
            df = pd.read_csv(file_path, sep='\t', names=column_names, header=0, low_memory=False)
            all_data.append(df)
