...
def has_displayed_quotes(line):
    # First check if the line has any quotes at all
    if '"' not in line:
        return False
    ...
    for part in parts:
        part = part.strip()
        # Only flag if quotes are within the field and not properly enclosing it
        if '"' in part:
            # Skip properly formatted CSV fields
            if not (part.startswith('"') and part.endswith('"')):
                # Verify it's not a legitimate CSV escape sequence
                if '""' not in part:
                    return True
    return False
...
def remove_quotes(file_path):
    ...
    for encoding in encodings:
        try:
            with open(file_path, 'r', encoding=encoding) as f:
                content = f.read()
            # Remove displayed quotes while preserving content
            ...
            if os.path.exists(temp_file):
                os.remove(temp_file)
            raise e
    raise Exception(f"Unable to process file {file_path} with any supported encoding")
