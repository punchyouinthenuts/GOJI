...
def fix_spanish_chars(text):
    ...
    # Split into words to check position
    words = text.split()
    fixed_words = []
    
    for word in words:
        # Keep uppercase if it's at start of word, otherwise lowercase
        for char, replacement in replacements.items():
            if char in word[1:]:  # Only replace if not first letter
...
def process_a_type(file_prefix, df, rollback_manager):
    ...
    df_a.to_csv(a_output, index=False, quotechar='"', encoding='latin1')
    df_pr.to_csv(pr_output, index=False, quotechar='"', encoding='latin1')
    ...
    sample_a.to_csv(a_proof, index=False, quotechar='"', encoding='latin1')
    sample_pr.to_csv(pr_proof, index=False, quotechar='"', encoding='latin1')
...
def process_ap_type(file_prefix, df, rollback_manager):
    ...
    df_ap.to_csv(ap_output, index=False, quotechar='"', encoding='latin1')
    df_appr.to_csv(appr_output, index=False, quotechar='"', encoding='latin1')
    ...
    sample_ap.to_csv(ap_proof, index=False, quotechar='"', encoding='latin1')
    sample_appr.to_csv(appr_proof, index=False, quotechar='"', encoding='latin1')
