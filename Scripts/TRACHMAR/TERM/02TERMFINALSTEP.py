...
try:
    ...
    # Convert mailed column to integer
    df_excel['mailed'] = df_excel['mailed'].astype(int)
    ...
    numeric_columns = ['Pallet Number', 'Tray Number', 'Bundle Number', 'Sort Position']
    for col in numeric_columns:
        if col in df_presort_print.columns:
            # Replace NaN with 0 before converting to int to avoid errors
            df_presort_print[col] = df_presort_print[col].fillna(0).astype(int)
    ...
    if check_network_path(network_base):
        # Create job number folder if it doesn't exist
        job_folder_name = f"{job_number}_{month}"
        job_folder_path = os.path.join(network_base, job_folder_name)
        
        # Check if any folder with this job number already exists
        existing_folders = [f for f in os.listdir(network_base) if job_number in f]
        
        if existing_folders:
            # Use the existing folder
            job_folder_path = os.path.join(network_base, existing_folders[0])
            print(f"Found existing job folder: {existing_folders[0]}")
        else:
            # Create new folder
            try:
                os.makedirs(job_folder_path, exist_ok=True)
                print(f"Created new job folder: {job_folder_name}")
            except Exception as e:
                print(f"Error creating job folder: {str(e)}")
        
        # Create HP Indigo/DATA subfolders
        indigo_data_path = os.path.join(job_folder_path, 'HP Indigo', 'DATA')
        try:
            os.makedirs(indigo_data_path, exist_ok=True)
            print(f"Created/verified HP Indigo/DATA folders")
        except Exception as e:
            print(f"Error creating HP Indigo/DATA folders: {str(e)}")
        
        # Copy the file
        dest_file = os.path.join(indigo_data_path, print_filename)
        
        if os.path.exists(dest_file):
            response = input("FILES ALREADY EXIST, OVERWRITE? Y/N: ").upper()
            if response == 'Y':
                shutil.copy2(presort_print_file, dest_file)
                print(f"Copied file to network (overwrite): {dest_file}")
            else:
                base, ext = os.path.splitext(dest_file)
                dest_file = f"{base}_copy{ext}"
                shutil.copy2(presort_print_file, dest_file)
                print(f"Copied file to network (as copy): {dest_file}")
        else:
            shutil.copy2(presort_print_file, dest_file)
            print(f"Copied file to network: {dest_file}")
    else:
        print("Network drive is offline. Skipping network copy operation.")

    # Clean up backup files BEFORE archiving
    cleanup_backups()
    print("Backup files cleaned up")

    # Move files to archive - MODIFIED to keep FHK_TERM_UPDATED.xlsx in work_dir
    # and skip any .bak files that might still exist
    for file in os.listdir(work_dir):
        ...
        # Skip the output file, we want to keep it in the original location
        if os.path.basename(src_file) == 'FHK_TERM_UPDATED.xlsx':
            # Copy the file to archive instead of moving it
            if os.path.exists(dst_file):
                base, ext = os.path.splitext(dst_file)
                dst_file = f"{base}_copy{ext}"
            shutil.copy2(src_file, dst_file)
            print(f"Kept copy of FHK_TERM_UPDATED.xlsx in work directory and copied to archive")
        elif os.path.isfile(src_file):
            # For all other files, move them to archive as before
            if os.path.exists(dst_file):
                base, ext = os.path.splitext(dst_file)
                dst_file = f"{base}_copy{ext}"
