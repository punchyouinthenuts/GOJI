...
# Create mask for 5-digit ZIP codes
five_digit_mask = ~df_pcexp['ZIP Code'].astype(str).str.contains('-')
five_digit_records = df_pcexp.loc[five_digit_mask, 'recno']

# Update the mailing status logic
df_main['mailed'] = df_main['recno'].map(
    lambda x: '14' if (x in five_digit_records.values) else 
              ('13' if x in df_pcexp['recno'].values else '14')
)
...
# Explicitly map the address fields including ZIP+4 with correct column names
