...
def normalize_text(text):
    ...
    if pd.isna(text):
        return ""
    # Convert to string, lowercase, and remove punctuation
    text = str(text).lower()
...
def normalize_address(address1, address2=""):
    ...
    # Sort words to handle order differences
...
for idx, main_row in df_main.iterrows():
    for move_idx, move_row in df_move.iterrows():
        # First check if names match (now using normalized names)
        name_match = main_row['norm_name'] == move_row['norm_name']
        
        # If names don't match exactly, try a contains check for hyphenated names, etc.
        if not name_match:
            # Check if one name contains the other (for partial matches)
            name_match = (main_row['norm_name'] in move_row['norm_name'] or 
                          move_row['norm_name'] in main_row['norm_name'])
        
        # If names match, check addresses
        if name_match:
            # Print diagnostic info about this potential match
            ...
            # For exact or contained addresses
            ...
            if addr_match:
                ...
                # Break after finding a match for this main record
                break
...
if matched_indices:
    ...
    for match in match_details:
        ...
        # Create and update city state zip
        city_state_zip = (
            str(match['move_city']) + ' ' + 
