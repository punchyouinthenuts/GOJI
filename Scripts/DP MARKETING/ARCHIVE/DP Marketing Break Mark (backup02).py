...
def display_headers_with_preview(df):
    ...
    for idx, (header, col) in enumerate(zip(df.columns, df.values.T), 1):
        ...
        row_idx = next((i for i, val in enumerate(col) if pd.notna(val)), 0)
...
def process_file(file_path):
    ...
    try:
        ...
        if df.iloc[0].notna().all():
            use_headers = input("\nFirst row appears to be headers. Use as headers? (Y/N): ").upper()
            if use_headers == 'Y':
                df.columns = df.iloc[0]
                df = df.iloc[1:].reset_index(drop=True)
            else:
                df.columns = create_headers(df)
        else:
            print("\nNO HEADER ROW! SET UP HEADERS")
            df.columns = create_headers(df)
        
        while True:
            display_headers_with_preview(df)
            if input("\nARE THESE HEADERS OK? (Y/N): ").upper() == 'Y':
                break
