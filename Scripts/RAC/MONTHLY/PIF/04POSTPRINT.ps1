...
function Undo-Changes {
    ...
    if (Test-Path $networkFolder) {
        $printFolder = "C:\Program Files\Goji\RAC\MONTHLY_PIF\FOLDER\PRINT"
...
try {
    ...
    # Define paths
    $baseFolder = "C:\Program Files\Goji\RAC\MONTHLY_PIF"
    ...
    $networkFolder = Join-Path $networkPath "$jobNumber $month PIF"
    
    # Create archive folder
    New-Item -Path $archiveFolder -ItemType Directory -Force
    
    # Move PROOFS zip file
    Move-Item -Path "$baseFolder\FOLDER\*PROOFS*.zip" -Destination "$baseFolder\FOLDER\PROOFS"
    
    # Copy folders to archive
    foreach ($folder in @('INPUT', 'OUTPUT', 'PRINT', 'PROOF')) {
        Copy-Item -Path "$baseFolder\FOLDER\$folder" -Destination $archiveFolder -Recurse
    }
    
    # Rename PDFs in PRINT folder
    $printFiles = Get-ChildItem "$baseFolder\FOLDER\PRINT\*.pdf"
    ...
    Move-Item -Path "$baseFolder\FOLDER\PRINT\*.pdf" -Destination $networkFolder
    
    # Previous code remains the same until the end section

    # Clear folder contents
    foreach ($folder in @('INPUT', 'OUTPUT', 'PRINT', 'PROOF')) {
        Get-ChildItem "$baseFolder\FOLDER\$folder" | Remove-Item -Recurse -Force
    }
