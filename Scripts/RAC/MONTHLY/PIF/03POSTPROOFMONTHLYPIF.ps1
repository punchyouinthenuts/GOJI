...
try {
    ...
    # Move and rename PDFs from OUTPUT with PIF check
    Get-ChildItem -Path $outputDir -Filter "*.pdf" | ForEach-Object {
        $originalName = $_.Name
        $newName = if ($originalName -notlike "*PIF*") {
            "$jobNumber $month PIF $originalName"
        } else {
    ...
    # Rename PROOF folder files with PIF check
