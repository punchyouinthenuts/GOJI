...
class TempFileManager:
    """Context manager for handling temporary files and directories with automatic cleanup."""
...
def detect_delimiter(file_path):
    ...
    try:
        ...
        return ','  # Default to comma as a fallback
...
def create_quoted_temp_file(original_path):
    ...
    with TempFileManager() as temp_manager:
        ...
        try:
            ...
            print(f"Error creating quoted temp file from {original_path}: {e}")
            raise
...
def create_temp_copy(original_path, new_name, temp_dir):
    ...
    try:
        ...
        print(f"Error creating temp copy of {original_path}: {e}")
        raise
...
def process_job_type(job_type, job_number, week_number, base_path, regen_files=None, version=None, postage_mapping=None):
    ...
    with TempFileManager() as temp_manager:
        try:
            ...
            print(f"Error processing {job_type}: {e}")
            # The TempFileManager will clean up temp files automatically
            raise
...
def main():
    ...
    parser.add_argument("--output_json", type=str, default="false", help="Whether to output JSON for database storage")
    ...
    try:
        with TempFileManager() as temp_manager:
            ...
            if args.job_type == "ALL":
                for job_type in JOB_TYPES:
                    try:
                        ...
                        print(f"Error processing job type {job_type}: {e}")
                        # Continue with other job types instead of failing completely
                        continue
            else:
                zip_filename = process_job_type(
                    args.job_type,
            ...
            if zip_files_created:
                ...
                print("No ZIP files were created. Please check the errors above.")
                sys.exit(1)
