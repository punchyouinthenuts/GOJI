...
# Function to save VERSION rows to FZAPO.csv and sample 15 rows
def save_version_rows(df, input_dir, proof_dir, file_name):
    df_version = df[df['VERSION'].notna()].copy()
    output_file = os.path.join(input_dir, "FZAPO.csv")
    ...
    sample_output_file = os.path.join(proof_dir, "FZAPO_PD.csv")
...
if file_name.endswith(".csv"):
    ...
    # Call the function to save VERSION rows, sample and update df
