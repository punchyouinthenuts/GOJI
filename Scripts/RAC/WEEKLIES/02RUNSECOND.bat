    ...
    for /f "tokens=1,2 delims=:" %%a in ("%%s") do (
        ...
        if "!ARG_TYPE!"=="positional" (
            if "!SCRIPT_NAME!"=="02EXCa.py" (
                python "%SCRIPT_DIR%\!SCRIPT_NAME!" "%JOB_NUM%" "%BASE_PATH%" "%EXC_POSTAGE%"
            ) else (
                python "%SCRIPT_DIR%\!SCRIPT_NAME!" "%BASE_PATH%" "%JOB_NUM%" "%WEEK%"
            )
        ) else (
            if "!SCRIPT_NAME!"=="06QUOTECHECK.py" (
                python "%SCRIPT_DIR%\!SCRIPT_NAME!" --base_path "%BASE_PATH%"
            ) else (
                python "%SCRIPT_DIR%\!SCRIPT_NAME!" --base_path "%BASE_PATH%" --job_num "%JOB_NUM%"
            )
        )
        if !errorlevel! neq 0 (
            echo.
